...
echo
echo "===== UBM TRAINING FOR SGMM2 PRE-TRAINING ====="
echo

START=$(date +%s);

steps/train_ubm.sh --cmd "$train_cmd" 500 data/train data/lang exp/tri2b_ali exp/ubm5b2 || exit 1;

END=$(date +%s);
UBM_TRAINING_TIME=$((END - START))
...
steps/train_sgmm2.sh --cmd "$train_cmd" 5000 12000 \
  data/train data/lang exp/tri2b_ali exp/ubm5b2/final.ubm exp/sgmm2_5b2 || exit 1;
END=$(date +%s);
SGMM_TRAINING_TIME=$((END - START))
...
START=$(date +%s);
utils/mkgraph.sh data/lang exp/sgmm2_5b2 exp/sgmm2_5b2/graph || exit 1
steps/decode_sgmm2.sh --config conf/decode.config --nj 1 --cmd "$decode_cmd" \
  exp/sgmm2_5b2/graph data/test exp/sgmm2_5b2/decode
END=$(date +%s);
SGMM_DECODING_TIME=$((END - START))
...
steps/align_sgmm2.sh --nj $nj --cmd "$train_cmd" \
  data/train data/lang exp/sgmm2_5b2 exp/sgmm2_5b2_ali || exit 1;
END=$(date +%s);
SGMM_ALIGNMENT_TIME=$((END - START))
...
echo "UBM" >>$log_file
echo $UBM_TRAINING_TIME | awk '{print int($1/60)":"int($1%60)}' >> $log_file

echo "SGMM" >> $log_file
echo $SGMM_TRAINING_TIME | awk '{print int($1/60)":"int($1%60)}' >> $log_file
echo $SGMM_DECODING_TIME | awk '{print int($1/60)":"int($1%60)}' >> $log_file
echo $SGMM_ALIGNMENT_TIME | awk '{print int($1/60)":"int($1%60)}' >> $log_file
